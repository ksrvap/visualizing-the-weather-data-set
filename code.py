...
def line_chart(df,period,col):
    ...
    df=df.groupby(df[period].dt.strftime('%m'))[col].mean()
    df.plot(kind='line')
    plt.show()
...
def plot_cont(df,plt_typ):
    ...
    for i in range(3):
        for j in range(2):
            if plt_typ=='distplot':
                sns.distplot(df[l[k]],ax=axs[i][j])
            elif plt_typ=='boxplot':
                sns.boxplot(df[l[k]],ax=axs[i][j])
            k+=1  
...
a={'Visibility (km)':'mean'}
group_values(weather_df,'Visibility (km)',a,'Weather')
